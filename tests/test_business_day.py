...
class TestBusinessDay:
    def test_5am_is_previous_day(self, app):
        """5:00 AM EAT (before 6:00 cutoff) → previous business day."""
        ts = datetime(2026, 6, 17, 2, 0, tzinfo=timezone.utc)  # 5:00 AM EAT (UTC+3)
        bd = business_day_for(ts)
        assert bd.day == 16

    def test_7am_is_current_day(self, app):
        """7:00 AM EAT (after 6:00 cutoff) → current business day."""
        ts = datetime(2026, 6, 17, 4, 0, tzinfo=timezone.utc)  # 7:00 AM EAT
        bd = business_day_for(ts)
        assert bd.day == 17

    def test_11pm_is_current_day(self, app):
        """11:00 PM EAT → current business day."""
        ts = datetime(2026, 6, 17, 20, 0, tzinfo=timezone.utc)  # 11:00 PM EAT
        bd = business_day_for(ts)
        assert bd.day == 17

    def test_configurable_cutoff(self, app):
        """Owner changes start hour to 4 → 5:00 AM is now current day."""
        with app.app_context():
            db.session.add(SystemSetting(key="business_day_start_hour", value="4"))
            db.session.commit()
    ...
    def test_bounds_use_eat_timezone(self, app):
        """Bounds for 2026-06-17 at cutoff 6 → start = 3:00 UTC (6:00 EAT)."""
        start, end = business_day_bounds("2026-06-17")
        assert start.hour == 3  # 6:00 EAT = 3:00 UTC
    ...
    def test_parse_date_bounds_uses_business_day(self, app):
        """The chokepoint function now returns business-day boundaries."""

                      ...
                      <motion.div key={item.id} whileTap={{ scale: 0.98 }}
                        ...
                        <div className="flex items-center gap-2 shrink-0">
                          ...
                          <button onClick={() => setDraft(d => ({ ...d, [item.id]: (d[item.id] ?? 0) + 1 }))}
                            className="w-9 h-9 rounded-full bg-emerald-500 text-white font-bold flex items-center justify-center">+</button>
                        </div>
                      </motion.div>
                    )
                  })}
            ...
            {stage === 'pay' && (
              <div className="space-y-4">
                  ...
                  {METHODS.map(m => (
                    <button key={m} onClick={() => setPay(p => ({ ...p, method: m }))}
                      className={`py-3 rounded-xl text-sm font-semibold border transition-colors ${
                        pay.method === m ? 'bg-emerald-500 text-white border-emerald-500' : 'text-white/60 border-white/10'
                      }`}>
                ...
                <input type="number" min="0" step="0.01" inputMode="decimal"
                  ...
                  className="w-full rounded-xl border border-white/10 bg-transparent px-4 py-3 text-white focus:outline-none focus:border-emerald-500" />

/**
 * DisputeQueueScreen — the manager's and owner's queue of formal complaints.
 *
 * This screen used to live in the EMPLOYEE app, where it showed a worker the
 *queue of everyone's complaints with Claim / Resolve / Dismiss buttons the
 * API refuses them, and no way to raise one of their own — which is the only
 * thing an employee actually needs from a dispute channel.
 *
 * The API already scopes the list: owner sees everything including owner-only,
 * a manager sees everything except owner-only, and an employee sees only their
 * own. So the same screen is safe for both readers here, and the employee app
 * keeps the filing form instead.
 */
...
function statusBadge(s: string): StatusValue {
  ...
  // 'resolved' / 'dismissed', not 'paid' / 'failed'. Borrowing the money words
  // because they were the green one and the red one stamped "Paid" on a
  // waiter's complaint about his shift rota, and "Failed" on one a manager had
  // considered and closed — which reads as the system failing, not a decision.

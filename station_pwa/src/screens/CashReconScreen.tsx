...
/** `embedded` renders this inside the Reconcile screen's Cash tab: the page
 *  header and role gate come from the parent, so they are dropped here to avoid
 *  showing two titles and gating twice. Standalone at /manager/cash it behaves
 *  exactly as before. */
export default function CashReconScreen({ embedded = false }: { embedded?: boolean } = {}) {
  ...
  const inner = (
      <motion.div
        ...
        transition={{ type: 'spring', damping: 25, stiffness: 300 }}
        className={embedded ? 'space-y-5' : 'p-4 max-w-3xl mx-auto space-y-5'}
      >
  ...
  // Embedded: parent already provides the gate and the boundary.
  if (embedded) return inner
  return (
    <RequireRole minLevel={5}>
      <ErrorBoundary level="tile">{inner}</ErrorBoundary>
    </RequireRole>
  )
}

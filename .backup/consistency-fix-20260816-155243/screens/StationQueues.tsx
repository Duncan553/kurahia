    ...
    <div className="p-5">
        ...
        <button
          ...
          className={`px-4 py-2 rounded-full text-sm font-semibold border transition-colors
            ${activeFilter === 'ALL'
              ...
              : 'border-white/10 text-ink-tertiary hover:text-[#f9dcd5] hover:bg-white/5'
            }`}
        >
      ...
      {items.length === 0 && (
        <div className="flex flex-col items-center justify-center py-20 gap-3">
          ...
          <p className="text-ink-tertiary text-lg font-medium">Queue clear</p>
        </div>
      )}
                ...
                >
                  ...
                  <div className="flex items-center justify-between mb-3">
                    ...
                    <span className={`inline-flex items-center gap-1.5 text-[11px] font-bold uppercase tracking-wider px-2.5 py-1 rounded-full
                      ${isActive
                        ? 'bg-primary-main/20 text-[#fa5c29]'
                        : 'bg-white/5 text-ink-tertiary'
                      }`}
                    >
                      {isActive && <span className="w-1.5 h-1.5 rounded-full bg-primary-main" />}
                  ...
                  <div className="flex-1 space-y-2.5 mb-4">
                    {group.items.map(item => (
                      <div key={item.order_item_id}>
                        ...
                        {/* Notes */}
                        {item.notes && (
                          <p className="text-xs italic text-ink-tertiary mt-0.5 ml-6">
                            - {item.notes}
                          </p>
                        )}
                  ...
                  <div className="flex gap-2 mt-auto">
                    ...
                    {/* Active (RECEIVED): show RECEIVE ALL + SERVE/COMPLETE */}
                    {isActive && (
                      <>
                        <motion.button whileTap={{ scale: 0.94 }}
                          onClick={() => {
                          ...
                          disabled={actMut.isPending}
                          aria-label={`Re-receive order ${ordLabel(group.order_id)}`}
                          className="flex-1 py-2.5 rounded-xl text-sm font-semibold
                            glass-card text-ink-tertiary
  ...
  return (
    <button onClick={toggle}
      ...
      className="w-9 h-9 rounded-xl glass-card flex items-center justify-center
        text-ink-tertiary hover:text-[#f9dcd5] hover:bg-white/5 transition-colors">
      {muted ? (
      ...
      <div className="sticky top-0 z-10 glass-card border-b border-white/10 px-5 py-3">
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            ...
            {count > 0 && (
              <span className="inline-flex items-center gap-2 bg-primary-main/15
              ...
              <button
                ...
                className={`px-3 py-1.5 text-xs font-bold uppercase tracking-widest transition-colors
                  ${view === 'queue'
                    ...
                    : 'text-ink-tertiary hover:text-[#f9dcd5]'
                  }`}
              >
                OVERVIEW
              </button>
